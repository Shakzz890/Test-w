...
const o = {
  ...
  // SearchField: qs('#SearchField'), // <-- REMOVED per user request
...
/* [FIX #3] 'touchend' listener for improved panel swipe */
  ...
  /* --- FIX: REMOVED SEARCHFIELD LISTENER --- */
  // (SearchField listener block was here, now removed)
  /* --- END FIX --- */
}
...
function buildNav() {
  ...
  const searchTerm = ''; // Search bar removed per user request
...
/* --- REMOVED SearchField listener block --- */

document.addEventListener('keydown', (e) => {

  // --- START CHANGE: Removed SearchField activeElement check ---
  /*
    (Block of code checking for document.activeElement === o.SearchField was here)
  */
  // --- END CHANGE ---
  ...
  if (bNavOpened) {
    ...
    if (bGroupsOpened) {
      ...
      // --- START CHANGE: Search Bar Navigation Removed & Bug Fix ---
      if (e.key === 'ArrowUp') {
          // Replaced buggy iCurrentChannel with iChannelListIndex
          if (iChannelListIndex > 0) {
              iChannelListIndex = (iChannelListIndex - 1 + aFilteredChannelKeys.length) % aFilteredChannelKeys.length;
              updateSelectedChannelInNav();
          }
          // (Removed logic for focusing SearchField)
      } else if (e.key === 'ArrowDown') {
          // Replaced buggy iCurrentChannel with iChannelListIndex
          if (aFilteredChannelKeys.length > 0) { 
          ...
          // (Removed logic for blurring SearchField)
      // --- END CHANGE: Search Bar Navigation ---
      } else if (e.key === 'Enter') {
            // --- START CHANGE: Bug Fix ---
            // Replaced buggy iCurrentChannel with iChannelListIndex
            ...
            // (Removed check for iCurrentChannel === -1)
            showGroups();
      }
  ...
  switch (e.key) {
    ...
    // --- START CHANGE: Bug Fix ---
    // Replaced buggy iCurrentChannel with iActiveChannelIndex
