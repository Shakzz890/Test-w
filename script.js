...
function selectGroup(index) {
  ...
  const afterTransition = (event) => {
    ...
    // Remove the listener to prevent multiple executions (should be handled by 'once', but belt-and-suspenders)
    ...
    if (aFilteredChannelKeys.length > 0) {
     ...
     // Keep Nav Open - No need to call showNav()
  };

  // Add the event listener *before* starting the transition
  // Use { once: true } to automatically remove listener after it fires once
...
/* -------------------------
    Settings & Modals
    ------------------------- */
// ... (Keep the Settings & Modals functions: renderChannelSettings, showVideoFormatMenu, hideVideoFormatMenu, renderVideoFormatMenu, setAspectRatio, togglePlaybackControls, showSettingsModal, window.hideSettingsModal, renderModalContent, window.applyChannelEdit, window.applyQualitySetting, window.setSubtitles, window.setAudio, toggleFavourite) ...
function renderChannelSettings() {
  ...
  try {
    if (player && typeof player.getActiveTracks === 'function') {
        ...
        if (activeTracks) {
          if (activeTracks.video && activeTracks.video.codec) vCodec = activeTracks.video.codec.split('.')[0].toUpperCase();
  ...
  if (o.CodecInfo) o.CodecInfo.textContent = `Video: ${vCodec} | Audio: ${aCodec}`;
...
function clearUi(exclude) {
  ...
  if (exclude !== 'guide') hideGuide(); // Uses window.hideGuide
...
/* -------------------------
    EPG
    ------------------------- */
// ... (Keep EPG functions: showEpg, hideEpg, renderEpg, generateDummyEpg) ...
function showEpg() {
  if (!o.EpgOverlay || !o.EpgChannels || !o.EpgTimeline) return; 
  clearUi('epg');
  iEpgChannelIndex = iCurrentChannel; 
  renderEpg(); 
  bEpgOpened = true;
...
function renderEpg() {
  ...
  let channelsHtml = '';
  // Ensure channels are filtered before rendering EPG list
  if (aFilteredChannelKeys.length === 0 && sSelectedGroup) buildNav(); 
  
  // Use a snapshot of filtered keys at the time EPG is opened
  const epgChannelKeys = [...aFilteredChannelKeys]; 

  epgChannelKeys.forEach((key, index) => {
    const ch = channels[key];
    if (!ch) return; 
...
/* -------------------------
    ...
    ------------------------- */
// ... (Keep Channel Name functions: showChannelName, hideChannelName) ...
...
/* -------------------------
    Favorites storage
    ------------------------- */
// ... (Keep Favorites functions: loadFavoritesFromStorage, saveFavoritesToStorage) ...
...
/* -------------------------
    ...
    ------------------------- */
// ... (Keep updateSettingsSelection function) ...
