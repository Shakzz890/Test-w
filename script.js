...
function applyPatchesToJW(playerId) {
  try {
    ...
    // 2) Disable pause via click/tap but keep touch events (double-tap) functional
    ...
    if (container) {
      ...
      const blockClickToggle = (e) => {
        // If user double-tap logic uses touch events, clicks will come later; still safe to stopImmediatePropagation
        e.stopImmediatePropagation();
        // Don't call preventDefault to allow focus/other behaviors.
      };
      container.addEventListener("click", blockClickToggle, true);
    }

    // 3) Override the player's pause API to be a no-op
    if (typeof player.pause === "function") {
      ...
      player.pause = function() {
        ...
        try { return false; } catch (err) { return undefined; }
      };
    }

    // 4) If player enters 'pause' state, immediately resume
