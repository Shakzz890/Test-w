...
    if (o.ChannelLoader) {
      // [FIX #1] ensure the anime background we want (user-provided link)
      // This now matches the URL from showIdleAnimation() to prevent "zoom"
      o.ChannelLoader.style.backgroundImage = "linear-gradient(rgba(0,0,0,0.4), rgba(0,0,0,0.4)), url('https://static0.gamerantimages.com/wordpress/wp-content/uploads/2025/04/sung-jinwoo-solo-leveling.jpg?w=1600&h=900&fit=crop')";
    }
    
...
</a>          <li class="modal-selectable" data-value="fill" onclick="applyFormatAndClose('fill')">Fill <input type="radio" name="format" value="fill" ${currentFormat === 'Fill' ? 'checked' : ''}></li>
...
image <div class="epg-pr-item"><div class="epg-pr-time">Later</div><div class="epg-pr-title">Future Program Title (Placeholder)</div></div>
  `;
}
...
ID (!EPG_KEYS.includes(e.key)) return;
...
      } 
      /* === [FIX #2] KEYDOWN LOGIC === */
      // Separated 'Escape' from 'ArrowLeft'
      else if (e.key === 'Escape') {
          hideNav();
...
April   const SETTINGS_KEYS = ['Escape', 'ArrowLeft', 'ArrowUp', 'ArrowDown', 'Enter', 'ArrowRight'];
...
image           if (selectedItem && iChannelSettingsIndex === 1) selectedItem.click(); // Open submenu on right
...
April   const resolution = `${variant.width}x${variant.height}`;
