...
var AppSettings = {
  ...
  defaultSettings: {
    ...
    "license-type": { type: "info", value: "Free" },
  },
  ...
  fillListLabels: function (sKey, oSetting) {
    ...
    if (aLabels && aLabels.length) {
      aLabels.forEach(function (oEl) {
        if (
          (sKey == "settings-password" || sKey == "password-groups") &&
          sValue
        ) {
          sValue = "****";
        }
  ...
  setInputFields: function (sKey, oSetting) {
    ...
    switch (sType) {
      case "radio":
        var oSetting = document.querySelector(
          "[data-name='" + sKey + "'][data-value='" + sValue + "']"
        );
        if (oSetting) {
...
function removeDeviceSettings() {
  ...
  aDeviceInclude.forEach(function (oEl) {
    if (
      oEl.dataset.device &&
      oEl.dataset.device.indexOf(sDeviceFamily) === -1
    ) {
      oEl.remove();
    }
  });
  ...
  if (
    localStorage.getItem("bIsPremiumApp") ||
    (typeof bIsPremiumApp === "boolean" && bIsPremiumApp)
  ) {
  ...
  if (AppSettings.isActive("store-epg-desciptions")) {
    getAvailableSpace(function (fUsed, fTotal) {
      ...
      if (fTotal < 40) {
        ...
        var oSetting = document.querySelector(
          "[data-name='store-epg-desciptions']"
        );
