...
function executeAction(sAction, oNavItem) {
  switch (sAction) {
    ...
    case "validate-license":
      ...
      if (
        sLicenseKey &&
        (sLicenseKey.length === 8 || sLicenseKey.length === 9)
      ) {
        validateLicenseKey(
          sLicenseKey,
          function (aResult) {
          ...
          function (oHttp) {
            ...
            if (oHttp.validationresult && oHttp.validationresult["error"]) {
              ...
              executeAction("check-license");
              showModal(
                getLang("validation-error"),
                oHttp.validationresult["error"]
              );
            } else {
              showModal(
                getLang("validation-error"),
                getLang("something-wrong")
              );
            }
          }
        );
    ...
    case "unlock-trial-license-ad":
      doAppServerRequest(
        "https://m3u-ip.tv/premium/trial-unlock.php",
        function (oHttp) {
    ...
    case "check-license":
      ...
      var oLicenseInfoElements =
        document.getElementsByClassName("current-license");
    ...
    case "calculate-space":
      var oSpaceInfoElements =
        document.getElementsByClassName("available-space");
      getAvailableSpace(
        function (fUsed, fTotal) {
          fTotal = fTotal - fUsed;
          if (fTotal > 90000) {
    ...
    case "clocksize": // show current time in guide area
      oSettingsGuide.innerHTML =
        '<span style="font-size: ' +
        oNavItem.dataset.value +
        '">' +
...
function openNavList(sListId, bIsPopStateEvent) {
  ...
  if (oCurrentNav.dataset.top) {
    oBreadCrumb.innerHTML =
      '<span id="bread_crumb_back">&laquo;</span>' +
      getLang(oCurrentNav.dataset.top);
...
function initControls() {
  ...
  document.addEventListener("keydown", function (e) {
    ...
    if (oActiveElement && oActiveElement.nodeName === "INPUT") {
      if (
        (k === 13 || k === 32 || k === 37 || k === 39) &&
        oActiveElement.className.indexOf("switch_input") > -1
      ) {
        e.preventDefault();
      ...
      switch (k) {
        ...
        case 40: // DOWN
          ...
          if (k == 38) {
            // UP
            iCurrentFieldIndex =
              (iCurrentFieldIndex - 1 + aCurrentFields.length) %
              aCurrentFields.length;
          } else {
            // DOWN
            iCurrentFieldIndex++;
    ...
    if (
      aCurrentFields &&
      (sActiveControl === "playlist" || sActiveControl === "epg")
    ) {
      switch (k) {
        case 13: // OK button (keyboard ENTER)
          if (
            oCurrentInputEditItem.nodeName === "INPUT" &&
            oActiveElement === oCurrentInputEditItem &&
            oCurrentInputEditItem.value
          ) {
            // allow to go to next field in next case
            k = 40;
          } else if (
            oCurrentInputEditItem.nodeName === "INPUT" ||
            oCurrentInputEditItem.nodeName === "SELECT"
          ) {
            oCurrentInputEditItem.focus();
            return true;
          } else {
        ...
        case 40: // DOWN
          ...
          if (k == 38) {
            ...
            if (
              ...
              oCurrentInputEditItem =
                aControlButtons[iCurrentControlButtonIndex];
              oCurrentInputEditItem.classList.add("select");
              return false;
            }
        ...
        case 37: // LEFT
          ...
          if (oCurrentInputEditItem.classList.contains("round-button")) {
            iCurrentControlButtonIndex =
              (iCurrentControlButtonIndex - 1 + aControlButtons.length) %
              aControlButtons.length;
          } else {
