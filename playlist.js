...
function getTypesCountOutput(oPlaylist) {
  ...
  if (oPlaylist.liveCount) {
    sHtml +=
      '<i class="icon-status icon-livetv">' + oPlaylist.liveCount + "</i>";
  }

  if (oPlaylist.movieCount) {
    sHtml +=
      ' <i class="icon-status icon-movies">' + oPlaylist.movieCount + "</i>";
  }

  if (oPlaylist.vodCount) {
    sHtml +=
      ' <i class="icon-status icon-movies">' + oPlaylist.vodCount + "</i>";
  }

  if (oPlaylist.seriesCount) {
    sHtml +=
      ' <i class="icon-status icon-series">' + oPlaylist.seriesCount + "</i>";
  }

  if (!sHtml) {
    sHtml =
      '<i class="icon-status icon-livetv">' + oPlaylist.channelCount + "</i>";
  }
...
function setPlaylistNavItem(oPlaylist) {
  ...
  if (oPlaylist.status === "ERROR" && oPlaylist.error) {
    ...
    sStatus =
      getTypesCountOutput(oPlaylist) +
      getLangTag("type") +
      ": " +
      oPlaylist.type.toUpperCase();
    localStorage.setItem("bReadyForPlay", "1");
  } else {
    sStatus = getLangTag("no-data-yet");
  }

  oPlaylistItem.innerHTML =
    sHtml +
    ' <i class="playlist-status">' +
    sStatus +
    '</i><span class="nav-item-loader"></span></p>';
  ...
  if (bDbInitiated && oDb) {
    ...
    oTx.oncomplete = function () {
      ...
      if (iPlayListCount) {
        ...
        oCountPlaylistsHint.innerHTML =
          getLangTag("count") + ": " + iPlayListCount;
      }
...
function setPlaylistToQueue(oPlaylist, bQueue) {
  ...
  var oListStatus = oNavItem
    ? oNavItem.querySelector(".playlist-status")
    : false;
  if (oListStatus) {
...
function openPlaylistManager(sId, sType) {
  ...
  if (sId === "new") {
    ...
    if (aLoadedPlaylists && aLoadedPlaylists[sId]) {
      ...
      oPmReloadPlaylist.classList.toggle(
        "checked",
        oCurrentEditPlaylist.startReload === true
      );
      ...
      if (oCurrentEditPlaylist.type === "xtream") {
        ...
        getEl("pm_xtream_load_vod").classList.toggle(
          "checked",
          oCurrentEditPlaylist.xtreamIncludeVod === true
        );
        getEl("pm_xtream_load_series").classList.toggle(
          "checked",
          oCurrentEditPlaylist.xtreamIncludeSeries === true
        );
      }
    } else {
...
function downloadXtream() {
  ...
  oCurrentEditPlaylist.url =
    sServerUrl +
    "get.php?username=" +
    sLogin +
    "&password=" +
    sPw +
    "&type=m3u_plus&output=" +
    sOutput;
  oCurrentEditPlaylist.epgUrl =
    sServerUrl + "xmltv.php?username=" + sLogin + "&password=" + sPw;
...
function downloadStalker(oPlaylist) {
  ...
  if (sStalkerMac) {
    document.cookie =
      "mac=" + sStalkerMac + "; Path=/; Secure; SameSite=None; HttpOnly=true";
  }
...
function addLegacyPlaylist() {
  ...
  iPlayListCount++;
  downloadPlaylistManager(
    oLegacyPlaylist,
    localStorage.getItem("sChannelListStorage")
  );
...
function downloadPlaylistManager(
  oPlaylist,
  sLocalData,
  sCallback,
  sCallbackError
  ...
  if (oPlaylist && window.Worker) {
    ...
    oActivePlaylistWorker.onmessage = function (e) {
      ...
      if (sResponseText) {
        ...
        if (sStatus) {
          // channels imported for this playlist
          oPlaylist.status = "OK";
        ...
        if (sStatus) {
          var sPlaylistEpgUrl = getMatch(
            sStatus,
            /(url-tvg|x-tvg-url|epg-url|tvg-url)="([^"]+)"/,
            2
          );
          if (sPlaylistEpgUrl && !oPlaylist.epgUrl) {
            // EPG URL found
