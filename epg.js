...
function setEpgNavItem(oEpg) {
  ...
  if (oEpg.status === "ERROR" && oEpg.error) {
    ...
    sStatus =
      getLangTag("channels-processed") +
      oEpg.channels +
      ", " +
      getLangTag("programs-processed") +
      oEpg.programs +
      sDate;
  } else {
...
function openEpgManager(sId, sType) {
  ...
  iCurrentFieldIndex = 0;
  aCurrentFields = getEl("em_url_form").querySelectorAll(
    "input, select, button"
  );
  ...
  if (oCurrentEditEpg.channels) {
    updateEpgDownloadStatus(
      getLang("channels-processed") +
        oCurrentEditEpg.channels +
        ", " +
        getLang("programs-processed") +
        oCurrentEditEpg.programs
    );
  }
}
    ...
    oActiveEpgWorker.onmessage = function (e) {
      ...
      if (sResponseText) {
        ...
        if (sStatus) {
          ...
          oEpg.status = "ERROR";
          oEpg.error = sStatus;
          oListStatus.innerHTML =
            '<span class="error">' + oEpg.error + "</span>";
          abortEpgDownload();
          saveCurrentEpg(oEpg);
          return true;
        }
        ...
        switch (sResponseText) {
          ...
          default:
            ...
            oListStatus.innerHTML =
              '<span class="error">' + sResponseText + "</span>";
        }
      }
    };
...
function saveCurrentEpg(oEpg, sCallback) {
  ...
  if (oDb && oEpg && oEpg.url) {
    ...
    oAddRequest.onerror = function (oEv) {
      if (
        sActiveControl === "epg" &&
        oEv.target.error.name === "ConstraintError"
      ) {
...
function updateEpgTimeShift(fValue) {
  getEl("epg_time_shift_output").innerText =
    "(" + parseFloat(fValue).toFixed(1) + " h)";
}
